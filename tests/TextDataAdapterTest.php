...
class TextDataAdapterTest extends KernelTestCase
{
    public function testDataAdapter()
    {
        $data = file_get_contents(__DIR__ . '/data/textAdapterTest.txt');

        $dataAdapter = new TextDataAdapter(PHP_EOL, "\t", '"');

        $result = $dataAdapter->prepare($data);
    ...
    {
        $data = file_get_contents(__DIR__ . '/data/textAdapterMultiDimensionalTest.txt');
